# CDK deployment of Sample Microservice System of Lambda Functions connected via multiple EventBrdige Buses
...
| Services     | EventBridge, Lambda, SimpleQueueingService                                                           |
| Integrations | CDK                                                                                                  |
...
![acrhitecture diagram showing the pipeline including Lambda producers, cross-region / cross-account EventBrdige buses and Sqs consumers](architecture-diagram.png)
...
Make shure to setup both accounts you want to deploy to acordingly.
...
- Set the environment variables in the .env file based on .env.example (if you want to deploy against AWS make shure to set the correct profile names for your aws cli configuration `ACCOUNT_PROFILE_NAME_SECONDARY=<name>` it must be the same as in your `.aws/config` file)
...
This will run a pytest defined in `tests/test_event_brdige_microservice.py`, there are two tests, one for cross-account and one for cross-region EventBridge buses, make shure that the correct stacks are deployed to the correct regions and accounts.
